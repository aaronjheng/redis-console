...
/// dependency is added. As an actor, all access is serialized on its executor;
/// the connection is additionally opened with `SQLITE_OPEN_FULLMUTEX`.
actor ShellHistoryStore {
    static let shared = ShellHistoryStore()

    /// Immutable after init, so the nonisolated deinit may read it safely.
    ...
    private nonisolated func createSchema(_ db: OpaquePointer) {
        let schema = """
